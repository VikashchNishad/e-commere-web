'use strict';
  ...
  function ShoppingCart(cartName) {
    ...
    // items in the cart
    //
    function CartItem(sku, name, discount, slug, mrp, price, quantity, image, category, packing, freeCount) {
        this.sku = sku;
        this.name = name;
          ...
          try {
              ...
              for (var i = 0; i < items.length; i++) {
                  ...
                  if (item.sku !== null && item.name !== null && item.price !== null) {
                      item = new CartItem(item.sku, item.name, this.item.discount ,item.slug, item.mrp, item.price, item.quantity, item.image, item.category, item.packing, item.status);
  ...
  ShoppingCart.prototype.addItem = function (sku, name, discount, slug, mrp, price, quantity, image, category, packing, freeCount) {
      ...
      if (quantity !== 0) {
          ...
          // new item, add now
          if (!found) {
              var itm = new CartItem(sku, name, discount, slug, mrp, price, quantity, image, category, packing, freeCount, 0);
              this.items.push(itm);
